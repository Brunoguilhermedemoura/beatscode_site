      ...
      <div className="bg-white rounded-xl max-w-md w-full p-8 relative" onClick={e => e.stopPropagation()}>
        ...
        {sent ? (
          <div className="text-center py-8">
            ...
            <button onClick={onClose} className="btn-primary mt-6">Fechar</button>
          </div>
        ) : (
          <>
            <h2 className="text-2xl font-bold mb-1">Solicitar demonstração</h2>
            <p className="text-gray-500 text-sm mb-6">Preencha e nossa equipe entra em contato.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input required type="email" placeholder="E-mail" value={form.email}
                ...
                className="w-full border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary"/>
              <input required placeholder="Telefone" value={form.phone}
                onChange={e => setForm({ ...form, phone: e.target.value })}
                className="w-full border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary"/>
              <input placeholder="Qual clube?" value={form.club}
                onChange={e => setForm({ ...form, club: e.target.value })}
                className="w-full border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary"/>
              <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}
                className="w-full border rounded-lg px-4 py-3 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-primary">
